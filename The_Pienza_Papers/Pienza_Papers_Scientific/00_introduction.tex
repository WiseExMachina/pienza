...
\begin{itemize}
    \item \textbf{Market Quality Index (MQI):} The normalization of market volatility used to isolate environmental quality from agent performance.
    \item \textbf{Efficient Frontier:} Determination of the rational search-time horizon where the accumulated cost of search offsets the probabilistic gain of future offers.
    \item \textbf{Fraud Prevention Mechanism:} An analysis of the structural logic regarding time-prediction variance and its role in mitigating moral hazard.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{\texttt{pienza\_big}:} A 1,000,000-row synthetic dataset generated via GANs. The Parquet artifacts are persisted on Google Drive and queried through BigQuery's external table interface to ensure zero-latency analysis.
    \end{itemize}
\end{itemize}
