...
\begin{tcolorbox}[
    ...
    \sffamily\small
    \textbf{METHODOLOGICAL PIVOT: THE FEATURE ABLATION STRIKE} \\
    \rule{12cm}{0.4pt} \\
    \vspace{0.2cm}
    \rmfamily
...
Layer 1 functions as a high-recall filter designed to isolate deterministic operational noise from strategic signal. By removing the majority classes (\textit{Non-Operational, Low Profitability}) at the entry point, the architecture prevents the ``Gravitational Well'' effect from cannibalizing the nuanced behavioral logic in the subsequent layer.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig_layer1_matrix.png} 
    \caption{Layer 1 Confusion Matrix. The diagonal illustrates the filter's efficiency in isolating noise. Note the high survival rate of the strategic signal (\texttt{THE\_NUANCED\_REST}).}
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.85\textwidth}{l c c X}
        \toprule
        \textbf{Layer 1 Metric} & \textbf{Value} & \textbf{Benchmark} & \textbf{Technical Verdict} \\
        \midrule
        \textit{Signal Retention} (Recall) & \textbf{81.7\%} & $>80\%$ & \textbf{PASS:} The strategic signal successfully bypasses the filter. \\
        \textit{Noise Removal} (Recall) & \textbf{90.6\%} & $>90\%$ & \textbf{PASS:} Efficient elimination of deterministic rejections. \\
        ROC-AUC (Triage) & 0.89 & $>0.85$ & \textbf{STABLE:} Strong discriminatory power for the initial split. \\
        \bottomrule
    \end{tabularx}
    \caption{Diagnostic Scorecard for Layer 1 (The Bouncer).}
    \label{tab:layer1_metrics}
\end{table}

\noindent \textit{Outcome: The Layer 1 filter successfully cleans the operational environment. By neutralizing 90.6\% of the low-level noise, the system ensures that Layer 2 operates on a high-signal sub-universe, providing the necessary data purity to capture the agent's strategic DNA.}


\subsection{Layer 2: The Strategist (Nuance Modeling)}
The second stage of the cascade operates exclusively on the high-yield subset that passed the Layer 1 filter. This model is designed to resolve the distinctions between rejections and acceptances without the interference of majority-class noise.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{fig_layer2_matrix.png} 
    \caption{Layer 2 Confusion Matrix. By isolating the high-yield sub-universe, the model achieves separation between complex decision states.}
    \label{fig:layer2_matrix}
\end{figure}

\subsubsection{Results: Performance Lift}
The transition to a hierarchical architecture resulted in a lift for the nuanced classes (Figure \ref{fig:layer2_matrix}):

\begin{itemize}[noitemsep, topsep=3pt]
    \item \textbf{Directional Intent (\texttt{strategic\_mismatch}):} Accuracy reached \textbf{75.0\%}, confirming the model mapped heuristics such as Friday gridlock and homecoming vectors.
    \item \textbf{Optimal Stopping (\texttt{expected\_value\_gamble}):} Accuracy rose to \textbf{65.9\%}, a 58\% relative improvement over the monolithic baseline.
    \item \textbf{Primary Target (\texttt{ACCEPTED}):} The model achieved \textbf{85.1\% accuracy} in replicating the agent's decisions, establishing a high-fidelity behavioral clone.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.85\textwidth}{l c c X}
        \toprule
        \textbf{Layer 2 Metric} & \textbf{Value} & \textbf{Benchmark} & \textbf{Technical Verdict} \\
        \midrule
        \textbf{F1-Macro (Champion)} & \textbf{0.76} & $>0.70$ & \textbf{WINNER:} 58.3\% lift over the monolithic baseline (0.48). \\
        ROC-AUC & \textbf{0.92} & $>0.90$ & \textbf{EXCELLENT:} High capability to rank opportunities vs. viable offers. \\
        Learning Curve Delta ($\Delta$) & 0.04 & $<0.05$ & \textbf{ROBUST:} Minimal gap between training and validation sets. \\
        \bottomrule
    \end{tabularx}
    \caption{Performance Ledger for Layer 2 (The Strategist).}
    \label{tab:layer2_metrics}
\end{table}

\noindent \textit{Outcome: The move to a hierarchical architecture results in an \textbf{F1-Macro of 0.76}. By removing the gravitational pull of majority classes, the model's accuracy for rejections and acceptances reached its predictive potential using purely autonomous features.}
...
\begin{figure}[H]
    \centering
    \textcolor{red}{\textbf{[PLACEHOLDER: ACTUALIZAR MATRIZ CAPA 1]}} \\
    \textit{Current state: fig\_layer1\_matrix.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Layer 1 Diagnostic Suite: Performance metrics for deterministic noise filtration.}
\end{figure}
...
\subsection{Layer 1: The Balanced Bouncer}
