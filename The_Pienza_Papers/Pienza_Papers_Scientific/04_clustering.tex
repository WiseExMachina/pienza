...
While pickups remain excluded from Phase 5 Supervised Learning, they are preserved for Phase 6 Graph-Neural Network (GNN) analysis and GAN-based manifold generation.
...
\begin{itemize}[noitemsep, topsep=3pt]
    \item \textbf{Realized Traffic Index (\texttt{realized\_traffic\_index}):} The post-facto ground truth of mission friction. It is calculated as the ratio of realized duration to the algorithmic expectation. This metric is strictly reserved for mission-completion audits.
...
\footnotetext{To maintain a strictly predictive environment, the \texttt{hist\_avg\_traffic\_index} and \texttt{traffic\_volatility\_index\_ML} were calculated using a \textit{No Look-Ahead Protocol}: only missions with a \texttt{completed\_timestamp} strictly earlier than the current \texttt{offer\_timestamp} were included in the rolling averages.}
...
\begin{table}[H]
    ...
    \begin{tabular}{rl l}
        ...
        \midrule
        10 & \texttt{pickup\_polygon\_id} & INTEGER \\
        12 & \texttt{pickup\_h3\_hex\_id} & TEXT \\
        \midrule
        1 & \texttt{dropoff\_polygon\_id} & INTEGER \\
        3 & \texttt{dropoff\_h3\_hex\_id} & TEXT \\
        4 & \texttt{dropoff\_hdbscan\_id} & INTEGER \\
        \midrule
