...
\subsection{Structural Discovery: Topological Analysis}

Following the coordinate remediation, the project executed a multi-stage unsupervised learning campaign to discover the city's organic operational zones. This process was not purely exploratory; it served to validate the Agent’s tacit spatial heuristics against a data-driven model.

\subsubsection{Geospatial Constraints and Selection Logic}
A fundamental architectural decision was the **exclusive focus on dropoff coordinates** for clustering. Pickup locations were intentionally omitted from the feature engineering phase for two reasons:
\begin{enumerate}
    ...
    \item \textbf{Decision Theory:} Analysis confirms that the Agent is geographically indifferent to pickups. The decision to accept an offer is driven by \textit{Time-to-Pickup} (TTP) rather than the specific neighborhood of origin. Conversely, the destination (dropoff) is the primary determinant of subsequent operational risk and return.
\end{enumerate}
\textit{Note: While pickups remain excluded from Phase 5 Supervised Learning, they are preserved for Phase 6 Graph-Neural Network (GNN) analysis and GAN-based manifold generation.}

\subsubsection{The Mapping Workflow: Iterative Convergence}
The discovery of the operational map followed a non-linear loop of discovery and correction:
\begin{itemize}[noitemsep]
    \item \textbf{Initial HDBSCAN:} Identified density clusters but exposed the systemic geocoding corruption described in Phase 2B.
    \item \textbf{Polygon Synthesis:} 72 heuristic polygons were manually architected to represent "Indifference Zones"---geographic areas where spatial variance is irrelevant to the Agent's strategy.
    \item \textbf{Delta Remediation:} The dataset was purified by calculating coordinate deltas against these polygons.
    \item \textbf{Canonical HDBSCAN:} The final unsupervised model was executed on the purified dataset, yielding the definitive topological backbone of the project.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{pienza_strategic_map.png} 
    \caption{Strategic Ground Truth: 72 hand-crafted heuristic polygons defining the Agent's operational indifference zones.}
...
\subsubsection{Emergent Urban Topologies}
The final HDBSCAN output (Figure \ref{fig:hdbscan_observatory_v2}) produced several key discoveries that were invisible to standard geocoding:
\begin{itemize}
    \item \textbf{The Tacubaya Emergence:} A distinct operational hub was discovered in \textit{Tacubaya} only after the coordinate surgery. Prior to remediation, these offers were incorrectly misallocated to the \textit{Juárez} sector.
    \item \textbf{Operational vs. Geometric Proximity:} The algorithm successfully identified the \textit{Haciendas / San Fernando} anomaly. While these zones are geographically adjacent, they are operationally distant due to terrain constraints. This confirmed that high-resolution fragmentation is a feature of the market's friction rather than a defect of the algorithm.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{rl | rl | rl | rl}
        ...
        4 & terminal\_1\_aicm & 15 & condesa & 26 & nodo\_const\_ref & 37 & interlomas\_magnocentro \\
        5 & terminal\_2\_aicm & 16 & nápoles\_wtc & 27 & santa\_fe\_patio & 38 & haciendas\_san\_fernando \\
        6 & centro\_historico & 17 & san\_miguel\_chapultepec & 28 & santa\_fe\_core & 39 & felix\_cuevas \\
...
\subsubsection{Ecological Validity and Overfitting Guardrails}
The resulting map does not attempt to model the total ride-hailing universe of Mexico City. It is a strictly constrained mirror of the \textbf{Agent's Operational Reality}. To maintain policy fidelity, the Agent never altered his heuristics to capture more data; consequently, the map reflects only the specific times and zones of expert-level deployment. 

The use of 72 granular polygons on a boutique dataset ($N \approx 4,700$) introduced a high risk of model overfitting. This was strategically managed during the supervised training phase through \textit{Feature Coalescence} and algorithmic regularization to ensure the model learned strategic principles rather than specific coordinate points.
