\section{Phase 2: Feature Synthesis \& Data Engineering}
...
\subsubsection{Generative Variability and Forensic Remediation}
...
For instance, the model occasionally produced compounded, hallucinated strings such as: \textit{``Turbo+ of \$16.00 and Turbo+ of \$14.07 are included...''} These unstructured artifacts rendered standard Regex parsing insufficient. Because these discrepancies threatened the integrity of downstream profitability analysis, a second iteration of \textbf{Manual Forensic Remediation} was executed. The Agent performed a line-by-line audit, cross-referencing corrupted records against the original image assets to ensure 100\% data purity for the final training set.
...
For analytical consistency, a work session is defined by its \textbf{Offer Window}—the duration between the inaugural offer and the terminal offer of a shift—rather than total physical operational time. To prevent \textbf{Label Contamination}, the window is programmatically truncated 15 minutes prior to the Agent’s arrival at the base in \textit{Anzures}. 

This heuristic ensures that the dataset minimizes "Terminal Rejections"---offers rejected solely due to shift termination rather than intrinsic trip quality. By isolating these events, the model is trained on \textbf{Active Economic Heuristics}, ensuring that the "Reject" labels in the primary training set remain a pure signal of low profitability or high operational risk, rather than a reflection of session closure.

Two primary features were calculated to quantify temporal positioning:
\begin{itemize}
...
\textbf{Status: Final Champion (Post-Phase 2).} This methodology involved calculating the \textbf{Cosine Similarity} between the \textit{Movement Vector} (Agent $\to$ Dropoff) and a fixed \textit{Home Vector} (Agent $\to$ \textit{Anzures}). While this feature eventually became the dominant spatial predictor in the \textit{Gold Palette} enrichment stage, the project relied on the Trial 3 Interpolation baseline for the duration of the Phase 2 structural engineering. This feature successfully quantified the Agent’s session-termination behavior.
...
This transformed the unstructured file repository into an audited relational ledger. The resulting high-fidelity temporal baseline unlocked the engineering of velocity-based features from Phase 2D, Silver Palette.
