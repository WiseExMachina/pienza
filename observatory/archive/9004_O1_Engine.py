...
# PAGE CONFIGURATION
# ==========================================
st.set_page_config(page_title="O(1) Engine Room (Legacy)", page_icon="\u26a1", layout="wide")
    ...
    with st.sidebar:
        ...
        try:
            with open("assets/Pienza_Papers.pdf", "rb") as f:
                pdf_data = f.read()
            st.download_button("\U0001F4C4 Download 91-Page Report (PDF)", data=pdf_data, file_name="Project_Pienza_Full_Report.pdf", mime="application/pdf")
        except FileNotFoundError:
            pass
        st.markdown("[\U0001F517 View GitHub Repository](https://github.com/your-repo)")
        st.markdown("---")
...
st.warning("This is a temporary page holding the legacy \'Dashboard \u2014 miniBabel O(1) Engine\' content during the 0008 page restructure. Will be removed once folded into the new P3 Latency Test.")
...
def render_sovereign_map(pred_name, true_name, google_coords, df_h3_master, geojson_data):
    ...
    COLOR_MISS = [100, 116, 139, 255]   # Slate (Miss) — no red, per design system
