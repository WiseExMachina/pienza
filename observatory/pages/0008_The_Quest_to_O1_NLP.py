...
""", unsafe_allow_html=True)
    ...
    _enc = (
        ...
        f'<div style="font-size:13.5px;color:#555;line-height:1.65;margin-bottom:6px;">Every token attends to every other token in the address simultaneously. Each head learns to specialize so the full context is captured before any classification happens.</div>'
        f'<div style="font-size:13px;color:#333;margin-bottom:14px;">4 heads · d_k = 32 each · scaled dot-product · Add &amp; Norm<span class="fn-wrap" style="margin-left:6px;"><span class="fn-mark" style="font-size:0.75rem;color:#21918c;font-weight:900;cursor:default;border-bottom:none;">ⓘ</span><span class="fn-tooltip" style="font-family:sans-serif;font-size:0.73rem;line-height:1.6;width:360px;"><b style="color:#21918c;">How the Attention Layer "Thinks"</b><br><br><b>4 Heads (d_k = 32):</b> Instead of looking at the address from a single perspective, the 128-dim bandwidth is split into 4 independent "heads" (32 dimensions each), allowing them to analyze the text simultaneously from multiple angles.<br><br><b>Scaled Dot-Product:</b> It uses a matrix dot-product to multiply the vectors of all words against each other. This operation measures the alignment between tokens, calculating exactly how strongly every single word should "pay attention" to every other word to capture the full context.<br><br><b>Add &amp; Norm:</b> It adds the original word data back into the output (so the model doesn\'t forget the original tokens) and normalizes the numbers to keep the neural network stable and learning efficiently.</span></span></div>'
        ...
        f'<div style="font-size:13px;color:#333;margin-bottom:14px;">Linear(128→256) → ReLU → Linear(256→128) · dropout 0.3 · Add &amp; Norm<span class="fn-wrap" style="margin-left:6px;"><span class="fn-mark" style="font-size:0.75rem;color:#21918c;font-weight:900;cursor:default;border-bottom:none;">&#9432;</span><span class="fn-tooltip" style="font-family:sans-serif;font-size:0.73rem;line-height:1.6;width:360px;"><b style="color:#21918c;">The Feed-Forward Workflow</b><br><br><b>Expansion:</b> The Linear(128→256) layer projects the token into a higher-dimensional space, learning complex combinations of the input features through its adjustable weights.<br><br><b>Filtering:</b> The ReLU activation acts as a non-linear filter, zeroing out negative values (noise) and letting only the strongest signals pass through.<br><br><b>Compression:</b> The second Linear layer compresses the refined data back to the standard 128 dimensions, applying another set of learned weights.<br><br><b>Regularization:</b> Dropout randomly disables 30% of connections to prevent the network from over-relying on specific signals.<br><br><b>Add &amp; Norm:</b> It adds the contextualized input back into the output (so the model doesn\'t forget the context it mapped during the Attention phase) and normalizes the numbers to keep the neural network stable and learning efficiently.</span></span></div>'
        f'<svg viewBox="0 0 620 100" style="width:100%;max-height:130px;display:block;overflow:visible;">'
        f'<text x="10" y="46" font-size="10" font-family="Courier New,monospace" fill="#888">x (128)</text>'
        ...
        f'<text x="130" y="54" font-size="8" font-family="Courier New,monospace" text-anchor="middle" fill="#888">128 → 256</text>'
        ...
        f'<text x="318" y="54" font-size="8" font-family="Courier New,monospace" text-anchor="middle" fill="#888">256 → 128</text>'
        ...
        f'</svg>'
        f'</div></div>'
        f'<p style="font-size:10.5px;color:#999;margin:10px 0 0;line-height:1.5;">The [30, 128] input tensor passes through both Transformer layers, where linear weights are updated via backpropagation to map the underlying structure of the sequence. The output is a context-aware [30, 128] matrix, passed directly to the Dual Pooling Fusion for structural compression.</p>'
    )
...
div[data-testid="stButton"].arch-pill > button:hover { background:#1a7a76; }
    ...
    _, _pill_col_back, _pill_col_fwd, _pill_col2 = st.columns([2, 1, 1, 1])
    with _pill_col_back:
        st.markdown('<div class="arch-pill">', unsafe_allow_html=True)
        _can_back = len(st.session_state['arch_history']) > 0
        if st.button('← Back', key='arch_back', disabled=not _can_back):
    ...
    with _pill_col_fwd:
        ...
        if st.button('Forward →', key='arch_fwd', disabled=not _can_fwd):
    ...
    with _pill_col2:
        st.markdown('<div class="arch-pill">', unsafe_allow_html=True)
        if st.button('Generate Random · Limit 100', key='arch_random'):
            addr = _fetch_random_address()
            if addr:
                st.session_state['arch_history'].append(st.session_state['arch_ex'])
                st.session_state['arch_future'] = []
