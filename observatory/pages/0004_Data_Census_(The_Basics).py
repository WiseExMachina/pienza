...
WHERE home_vector_alignment_score IS NOT NULL
  AND session_progress_ratio IS NOT NULL;""",
    ...
    "Market Quality Index": f"""-- MQI: eph_operational per offer, session, and category
SELECT
    ml.offer_id,
    ml.session_fk,
    ...
    ml.eph_operational
FROM `{PROJECT}.{DATASET}.v_ML_Supervised` ml
LEFT JOIN `{PROJECT}.{DATASET}.product_category` pc
  ON pc.product_category_id = ml.product_category_fk
WHERE ml.eph_operational IS NOT NULL
  AND ml.session_fk IS NOT NULL;""",
...
<div style='background:#fff;border:1px solid #e2e8f0;border-top:3px solid {color};
 border-radius:8px;padding:14px 16px;'>
  ...
  <div style='font-size:0.65rem;color:#b0bec5;margin-top:5px;'>IQR {q1:.0f} – {q3:.0f}</div>
            ...
            elif active_pill == "Market Quality Index":
                import plotly.graph_objects as go
                import numpy as np
                import pandas as pd
                ...
                def _mqi_color(val, vmin, vmid, vmax):
                    """Monochromatic teal: muted gray-teal below 1.0, saturated teal above."""
                    if val <= vmid:
                ...
                df["simplified_category"] = df["category"].apply(_map_cat_mqi)
                df["eph_operational"] = pd.to_numeric(df["eph_operational"], errors="coerce")

                df_core = df[df["simplified_category"].isin(["X", "Mid-tier", "Black"])].dropna(subset=["eph_operational"]).copy()
                _cap99 = float(np.percentile(df_core["eph_operational"].values, 99))
                df_core = df_core[df_core["eph_operational"] <= _cap99]

                anchors = df_core.groupby("simplified_category")["eph_operational"].median().to_dict()
                df_core["iq"] = df_core["eph_operational"] / df_core["simplified_category"].map(anchors)
...
""", unsafe_allow_html=True)
                ...
                _p97 = float(np.percentile(df_core["eph_operational"].values, 97))
                _p02 = float(np.percentile(df_core["eph_operational"].values, 2))
                ...
                for cat in CAT_ORDER:
                    ...
                    v_eph = df_core[df_core["simplified_category"] == cat]["eph_operational"].values
                ...
                for cat in CAT_ORDER:
                    color = CAT_COLORS[cat]
                    v_iq  = df_core[df_core["simplified_category"] == cat]["iq"].values
                    anchor = anchors.get(cat, 1.0)
                ...
                fig_after.add_hline(y=1.0, line_dash="dash", line_color="#475569",
                ...
                st.markdown(
                    "<div style='font-size:0.9rem;font-weight:700;color:#334155;"
                    "margin:20px 0 2px;'>Session Quality Timeline</div>"
                    "<div style='color:#888;font-size:0.82rem;margin:0 0 8px 0;'>"
                    "Each box is one session. Darker teal = above the 1.0 baseline; "
                    "lighter teal = below.</div>",
                    unsafe_allow_html=True,
                )

                mqi_sess = df_core.groupby("session_fk").agg(
                    MQI=("iq", "mean"),
                    N=("eph_operational", "count"),
                ).reset_index()
                mqi_sess = mqi_sess[mqi_sess["N"] >= 10].sort_values("session_fk")

                if mqi_sess.empty:
                    st.warning("Not enough sessions with 10+ offers to render timeline.")
                else:
                    vmin_s = float(mqi_sess["MQI"].min())
                    vmax_s = float(mqi_sess["MQI"].max())

                    df_sm = df_core[df_core["session_fk"].isin(mqi_sess["session_fk"])].merge(
                        mqi_sess[["session_fk", "MQI"]], on="session_fk"
                    )

                    fig_sess = go.Figure()
                    for sess_id in mqi_sess["session_fk"]:
                        sd = df_sm[df_sm["session_fk"] == sess_id]
                    ...
                    fig_sess.update_layout(
                        ...
                        showlegend=False,
                        yaxis=dict(
                            title="IQ", range=[0.4, 2.6], zeroline=False,
...
The market is deeply heterogeneous — both <em>across</em> sessions and <em>within</em> them.
</div>""", unsafe_allow_html=True)
