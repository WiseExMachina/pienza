...
# Layout Refinement (Ejes, fondo y leyenda idénticos a tu referencia)
fig.update_layout(
    ...
    yaxis=dict(
        range=[0.6, 1.3], 
        gridcolor='lightgrey', # Cuadrícula visible
        zeroline=False
    ),
    xaxis=dict(
        gridcolor='lightgrey', # Cuadrícula visible
        tickformat="%b-%d"     # Formato de fecha (ej. Aug-22)
    ),
    
    height=600,
    margin=dict(l=60, r=20, t=40, b=50),
    
    # LEYENDA: Arriba a la izquierda, con fondo blanco y borde (como en la foto)
    legend=dict(
        yanchor="top", y=0.98, 
        xanchor="left", x=0.01,
        bgcolor="white",
