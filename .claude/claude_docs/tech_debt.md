...
- **2026-08-01 — RAG evaluation, no scope defined yet.** Flagged verbally as a weekend/future item ("mandatory RAG evaluation") but never written down as an actual tech-debt entry until now — recovered from conversation history after being caught missing. No framework or approach decided (RAGAS-style automated metrics vs. a golden-question-set + LLM-judge vs. something else) — purely a placeholder to not lose the intent again. Natural to scope once the retrieval-recall gap above and query rewriting are addressed, since evaluation would want to measure exactly those failure modes (did retrieval surface the right chunk, did the answer stay grounded) across all 4 corpora.
