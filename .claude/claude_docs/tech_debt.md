...
- **2026-08-01 — RAG Assistant (0010) answered a question by surfacing the ITESM Data Science Certificate and interview framing — reads as a student project, needs scrubbing.** Live-tested question "Is the scope of the project that of an entry level data scientist?" returned an answer citing that Project Pienza was "built alongside the ITESM Data Science Certificate" and is "explicitly framed as a portfolio project for an AI Engineering role interview in the insurance/claims sector." Even if technically true, surfacing this framing to anyone probing the deployed RAG (a recruiter, an interviewer) undercuts the project by making it read as coursework/interview-prep rather than professional-caliber work standing on its own. Two parts to fix:
  1. Find and scrub explicit ITESM/certificate and interview-framing language from whichever doc(s) the retrieved chunk actually came from. Grepped `.claude/claude_docs/` and `assets_ignored/claude_docs/` for "ITESM"/"certificate" — no direct hits in either tree as of this entry, so the exact source chunk is still unconfirmed; likely candidates flagged by a broader grep for "portfolio project"/"AI Engineering"/"insurance/claims"/"neoris": `Agentic_Knowledge.md`, `files_dict.md`, `MEMORY.md`, `session_ledger.md`, `placeholder.md`, `rag_workflow.md` (all under `assets_ignored/claude_docs/`, the currently-live corpus source per the already-tracked stale-corpus item above). Needs a follow-up session to actually trace the exact retrieved chunk (e.g. via `retrieve()` directly against the loaded parquet) rather than guessing from a grep.
  2. Once found, decide: scrub the framing entirely from files that would ever be embedded in a public-facing corpus, or exclude certain docs (changelog-style ones like `session_ledger.md`/`tech_debt.md`) from the RAG #1 corpus build going forward — these are meant as internal working history, not public-facing content, and probably shouldn't be embedded verbatim regardless of this specific leak.
  - This is a good argument for finally executing the already-tracked "RAG #1 corpus is stale" item (repoint `DOCS_DIR` from `assets_ignored/claude_docs/` to `.claude/claude_docs/`) — doing that rebuild is also the natural point to decide which files belong in a public-facing corpus at all.
