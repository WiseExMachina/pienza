...
- **2026-08-01 — Candidate #6: RAG over the Pienza codebase itself.** Discussed conceptually (not scoped/built): treat the repo's own source (`observatory/`, `.claude/claude_docs/`, etc.) as a corpus, so questions like "where does the anonymization logic live?" or "what calls `retrieve_trips()`?" can be answered via retrieval instead of manual grep. Chunking strategy would differ from markdown/tabular: by function/class (ideally AST-based per language, not naive line-split) or by file, with metadata (filename, function/class name, imports) mattering more than in free text. Hybrid search (keyword/BM25) likely more valuable here than in prose corpora, since exact symbol names matter. Good candidate to discuss conceptually even if not built — demonstrates chunking generalizes beyond text/tabular when adapted to the domain.
