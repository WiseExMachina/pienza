...
- **DONE (2026-08-01) — Combined task: rebuild RAG #1's corpus pointed at the canonical path, scrub coursework/job-search framing, and automate future rebuilds.** Merged from two separate entries once it became clear they're the same fix.
  ...
  5. **Automated going forward**: new `.github/workflows/rebuild-rag-corpus.yml` (nightly cron + `workflow_dispatch`) reruns steps 1-4's rebuild+upload automatically, so the corpus never goes stale like this again without someone remembering to re-run it manually. Cost checked before building (not assumed): full tree is 310,706 chars, ~$0.008/rebuild, ~$0.23/month nightly — negligible. This required a scoped, documented exception to root `CLAUDE.md`'s "never write to GCS without explicit permission" rule (added same day) — the exception is limited to this one fixed, reviewed, single-purpose job, not a general loosening. **Still needs a human action, not mine**: the GitHub repo secret `GCP_SERVICE_ACCOUNT_KEY` must be set (`gh secret set GCP_SERVICE_ACCOUNT_KEY < path/to/key.json`) before the workflow can run — untested end-to-end until that secret exists and a manual `workflow_dispatch` run is confirmed green.
