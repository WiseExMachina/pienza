...
- **DONE (2026-08-01) — Combined task: rebuild RAG #1's corpus pointed at the canonical path, scrub coursework/job-search framing, and automate future rebuilds.** Merged from two separate entries once it became clear they're the same fix.
  1. `observatory/scripts/rag_build_corpus.py` repointed `DOCS_DIR` from `assets_ignored/claude_docs/` (untracked, predates the CLAUDE2 reorg) to the real canonical `.claude/claude_docs/`. `EXCLUDE` emptied — the old personal-file exclusions (`STAR_stories.md` etc.) don't exist in the new tree, they live entirely under `assets_ignored/interview_prep/` now.
  2. Job-search/coursework framing scrubbed from `rag_workflow.md` (title, frontmatter, prose), `session_ledger.md` (changelog entries), `agentic_knowledge.md`, `tech_debt.md`, and `files_dictionary.md` — technical content preserved throughout, only the framing reworded to read as professional project documentation. The specific certificate name that appeared in the original leaking answer was never found via grep in either doc tree, so that detail was likely synthesized by the model (default temperature 1.0, kept deliberately per an earlier decision) rather than retrieved verbatim — but the real, confirmed job-search framing that WAS baked into the docs is now gone.
  3. **Real bug found and fixed along the way, in shared infra**: rebuilding against the new corpus hit a 400 error Vertex AI's `text-embedding-004` had not surfaced before — a **20,000-token-per-request ceiling**, separate from the already-known 250-instance ceiling. A 200-chunk batch of this corpus's denser technical markdown (code blocks, paths, symbols) totaled 72,489 tokens. Measured real ratio: **2.85 chars/token** for this content (denser than plain prose). Fixed in `observatory/utils/vertex_embed.py`: `embed_batch()` now transparently sub-batches on whichever limit binds first (instance count via existing `BATCH_SIZE=200`, or a new conservative `CHAR_BUDGET=42000`), so callers (`rag_build_corpus.py`, `rag_build_corpus_paper.py`, `rag_build_vectordb_trips.py`) never need to know either ceiling exists — fixed once in the shared module, benefits all three.
  4. Rebuild ran clean after the fix: 322 chunks from 17 files, `(322, 5)` shape, 768-dim embeddings, verified with pandas. Uploaded to GCS (`gcs_deploy.py --page 0010`, dry-run confirmed 1 real change first). **Verified live**: re-asked the exact question that originally leaked the framing ("Is the scope of the project that of an entry level data scientist?") — the model now correctly says it cannot answer from the retrieved context, and (correctly) cites the tech-debt entry documenting that the framing was removed, instead of fabricating an answer.
  5. **Automated going forward — verified working end-to-end.** New `.github/workflows/rebuild-rag-corpus.yml` (nightly cron + `workflow_dispatch`) reruns steps 1-4's rebuild+upload automatically, so the corpus never goes stale like this again. Cost checked before building (not assumed): full tree is 310,706 chars, ~$0.008/rebuild, ~$0.23/month nightly — negligible. Required a scoped, documented exception to root `CLAUDE.md`'s "never write to GCS without explicit permission" rule (added same day), limited to this one fixed, reviewed, single-purpose job. First manual `workflow_dispatch` run failed (`GCP_SERVICE_ACCOUNT_KEY` secret had been saved empty in the GitHub UI — real user error, not a workflow bug); re-saved correctly, second run went green in 3m39s. Confirmed via the GCS blob's own `updated` timestamp (matches the run window) that the automated pipeline actually uploaded, not just reported success. Also added: also gained `gh` CLI in this Codespace along the way (wasn't installed), useful going forward for triggering/inspecting Actions runs without leaving the terminal — though the Codespaces auto-token lacks `secrets` scope, so reading/listing repo secrets still requires the GitHub web UI or an interactively re-authenticated `gh` session.
  6. **DONE (2026-08-02) — found a second, later leak of the same framing, this time inside the system prompts themselves, not the corpus docs.** `SYSTEM_PROMPT`, `REASONING_SYSTEM_PROMPT`, and the tone-guidance line in `SYSTEM_PROMPT` (all in `_0010_data.py`) still described the page as "built to showcase RAG architecture for an AI Engineering interview in the insurance/claims sector" and called itself "an interview-prep demo" — this survived the 2026-08-01 corpus/docs scrub because it was never in a doc file, it was hardcoded directly into the prompt strings. User caught it live while reading a rendered "View system prompt" expander (the new Reasoning Mode feature, see the toggle entry below — ironically the transparency feature that was built to demonstrate honest disclosure is what surfaced this). Fixed: reworded to "a Streamlit Observatory built to showcase RAG architecture" / "a technical portfolio demo", no interview/employer-sector framing left in either prompt. Confirmed via `grep -rn "interview\|AI Engineering\|insurance/claims"` across both `_0010_data.py` and `0010_RAG_Assistant.py` — zero matches after the fix.
  7. **DONE (2026-08-02) — audited all 3 system prompts (`SYSTEM_PROMPT`, `REASONING_SYSTEM_PROMPT`, `AGENTIC_SYSTEM_PROMPT`) against Anthropic's published prompt-engineering best practices** (fetched live from claude.com/blog/best-practices-for-prompt-engineering, not recalled from training). Confirmed already-compliant: explicit/direct instructions, context/motivation behind rules (e.g. the grounding rule explains *why* — "a wrong confident answer is worse than an honest..."), explicit permission to express uncertainty (the grounding rule and the agentic prompt's "say so plainly if the retrieved context doesn't answer the question" both already do this), and concrete examples via `e.g.` throughout. **One real gap found and fixed, surgically**: several instructions were phrased purely negative ("do not X", "decline", "never Y") without an equivalent positive framing — the article's "tell the AI what TO do instead of what NOT to do" rule. Rewrote ~6 instructions across the 3 prompts (corpus-scope redirect, style guidance, off-topic handling, prompt-injection handling, agentic tool-call sequencing) to lead with the desired positive behavior while preserving the exact same rule/constraint — no behavior change intended, verified live via Playwright that off-topic decline still works correctly (clear decline + redirect to actual scope). Deliberately did not do a full rewrite (a "start simple, add complexity only when needed" call per the same article) since the prompts already scored well on every other practice.

- **DONE (2026-08-01) — Agentic RAG tab added to 0010 (RAG Assistant), real tool-use routing, not cosmetic.** The page now has two top-level tabs: "Manual Retrieval" (the existing 3-corpus stepper, unchanged) and "Agentic RAG" (new — a single chat thread with no stepper, where Claude itself picks which corpus to query via Anthropic tool use). `ask_claude_agentic()` in `_0010_data.py` builds one tool per corpus from `CORPORA` (`query_claude_docs`/`query_paper`/`query_trips`), lets Claude choose exactly one per question, executes retrieval via the same `retrieve()`/`retrieve_trips()` helpers Manual Retrieval already uses (via new `_retrieve_for_corpus()` dispatcher), then generates a grounded final answer. The UI shows a `Queried: <corpus label>` pill badge per answer so the routing decision is visible, not a black box. Verified live via Playwright: a Trip-Records-shaped question correctly routed to Trip Records, a docs-shaped question correctly routed to Project Docs, both grounded in real retrieved rows, no tracebacks.
  - **Real bug found and fixed along the way**: the corpus-stepper's CSS that hides the inner (hidden, JS-driven) `st.tabs()` tablist — `[data-baseweb="tab-list"] { display:none !important; }` — was unscoped, so once an outer `st.tabs()` was added it hid BOTH the inner corpus tablist and the new outer Manual-Retrieval/Agentic-RAG tablist. Fixed by scoping the selector to `[data-baseweb="tab-panel"] [data-baseweb="tab-list"]` (only hides tab-lists nested inside a tab-panel, i.e. the inner one). Also fixed the stepper's driving JS, which queried `[data-baseweb="tab"]` globally via `window.parent.document.querySelectorAll(...)` with no scoping — broke index-matching once outer tabs existed. Fixed by scoping to the closest `[data-baseweb="tab-panel"]` ancestor of the `.corpus-stepper` div.
  - **Also found and fixed**: the system prompt (`SYSTEM_PROMPT` in `_0010_data.py`) described all 3 corpora generically as if a single Manual-Retrieval chat thread could answer about any of them — caused Claude to answer meta-questions like "what can I ask?" by reciting all 3 corpora as options, reading as agentic routing that didn't actually exist in that mode. Fixed by parametrizing the prompt with `{corpus_label}` (formatted per-call via `ask_claude(..., corpus_label)`), so each Manual-Retrieval thread only ever describes itself as scoped to its one active corpus.
  - **Deferred, logged here per explicit user decision — the "pro" version of agentic routing, not built now**: multi-tool calls in a single turn (a question spanning 2+ corpora), parallel tool execution, and streaming. Current MVP is single-tool-per-question — still genuinely agentic (a real routing decision, not scripted), just not exhaustive. Revisit if a future showcase calls for demonstrating a more advanced agent loop.

- **DONE (2026-08-02) — Candidate #6: RAG over the Pienza codebase itself, built as a 4th corpus (C4/"Codebase") on 0010.** New `observatory/scripts/rag_build_corpus_codebase.py`: reads `observatory/{pages,utils,components,scripts}/*.py` + the 3 root files (`main.py`/`_main_data.py`/`config.py`), deliberately excludes `archive/` (deprecated pages). Chunking is real AST-based (Python's `ast` module), not line-split: one chunk per top-level function/class (with a metadata header naming file/symbol/imports), plus one "module-level" chunk per file for anything outside a def/class. 40 files -> 147 chunks, 712,928 chars -> ~250K tokens (2.85 chars/token, this project's measured ratio for dense technical content). Embedded via the same shared `vertex_embed.py` (1 batch, well under both the 250-instance and 20K-token ceilings), written to `data/dumped_files/rag_corpus_codebase.parquet`, same 5-column schema as the other parquet corpora — a drop-in 4th entry in `CORPORA` (`_0010_data.py`), no changes needed to `retrieve()`/`ask_claude()`. `AGENTIC_TOOLS` picked it up automatically (built via list comprehension over `CORPORA`); only `AGENTIC_SYSTEM_PROMPT` needed a manual edit (3 corpora -> 4, added `query_codebase` description). Uploaded to GCS via `gcs_deploy.py --page 0010` after adding a new MANIFEST entry (user explicitly authorized this one upload, per the repo's GCS-only-with-permission rule). **Verified live via Playwright**: C4 "Codebase" shows correctly in the Manual Retrieval stepper with its token count; a direct Codebase-tab question ("where is retrieve_trips defined") returned a correct, grounded answer citing `pages/_0010_data.py` with an accurate function-by-function description; an Agentic RAG question about the routing mechanism itself correctly self-referentially routed to `QUERIED: CODEBASE` and accurately described `ask_claude_agentic()`'s real tool-use loop. No tracebacks, no UI regressions on the other 3 corpora.
  - **Deferred, not built now**: hybrid search (keyword/BM25) — flagged by the user as valuable here specifically since exact symbol names matter more than in prose corpora, tracked as a cross-corpus future item (see the standing "hybrid search en todo" note). AST-based chunking for non-Python files (JS/AppsScript in `research_full/`, if that ever gets its own corpus) not attempted — this corpus is Python-only by design (`observatory/` is 100% Python).

- **DONE (2026-08-01) — Conversational memory added to the RAG Assistant (0010), with real compaction.** `ask_claude()` (`_0010_data.py`) now takes `history`/`summary` params; each of the 3 corpus tabs has its own `st.session_state`-backed chat thread (`st.chat_input`/`st.chat_message`), a per-tab "Clear conversation" button, and a `.fn-wrap` tooltip disclosing the design (sliding window of `MAX_HISTORY_TURNS = 6` verbatim turns, older turns folded into a running LLM-summarized rollup via `compact_if_needed()` instead of silently dropped — a deliberate "simulate a scaled environment" choice, not required at this corpus size). Retrieved-context blobs are never resent in history (only question/answer text pairs), which is the actual cost lever — re-sending full context every turn would multiply cost ~linearly with conversation length. Query rewriting for follow-ups at retrieval time was explicitly deferred (separate item, see below) — retrieval still embeds only the raw latest question, and memory alone was enough for the model to answer follow-ups correctly (verified live: "what about the fare amount rule specifically?" after an addresses question, correctly resolved without repeating the topic).
  - **Also resolved the same day: system prompt best practices pass** (was a separate open item below). `SYSTEM_PROMPT` was rewritten from a one-line generic string into a structured prompt covering role/corpus disambiguation, grounding rule, citation rule, style, conversational-memory-vs-grounding distinction, edge-case handling (multi-part questions, contradicting passages, off-topic questions, prompt-injection attempts), tone, and per-corpus answer-format guidance.
  - **Prompt caching was attempted, then explicitly descoped by the user mid-implementation.** Real finding, not a guess: the Plan agent's assumption of a 1024-token minimum cacheable prefix for Haiku 4.5 was wrong — WebFetch against the live Anthropic docs confirmed the real minimum is **4,096 tokens** (4x higher), and no error is returned when under it — `cache_creation_input_tokens`/`cache_read_input_tokens` just silently stay 0. Verified empirically via direct API calls at 923, 1214, 1563 tokens — all cached 0 bytes, consistent with the doc once the real threshold was known. User's call: don't chase the token minimum for a demo-scale prompt, keep things atomic/non-black-box, revisit prompt caching as its own separate future task rather than bundling it into the memory work. `cache_control` was removed from `_claude_request()`'s system field (reverted to a plain string); `ask_claude()` returns `(answer, updated_summary)`, no cache-usage tuple member.
  - Verified live via a real Playwright smoke test (`chromium`, headless) against a fresh local Streamlit instance: multi-turn conversation with memory, citations expander, timing caption, and no tracebacks in the browser console or page body.

- **DONE (2026-08-02) — Strict Grounding vs Reasoning Mode toggle added to Manual Retrieval (0010), per-tab.** Followed directly from the entry-level-leak incident retrospective: the fix that day was two things, not one (corpus scrub + a new anti-extrapolation grounding rule in `SYSTEM_PROMPT`) — this makes that second half a visible, demonstrable trade-off instead of a silent one. New `REASONING_SYSTEM_PROMPT` in `_0010_data.py` (Reasoning Mode): explicitly permitted to extrapolate/infer beyond retrieved context, but must disclose when it does (transparency rule) and still cite sources when it draws on retrieval. `ask_claude()` gained a `mode: str = "strict"` param selecting between `SYSTEM_PROMPT`/`REASONING_SYSTEM_PROMPT`. UI: each of the 3 Manual Retrieval tabs (Project Docs/Papers/Trip Records — Codebase inherits the same mechanism) got its own `st.toggle("Reasoning Mode")` plus an "Active: <mode>" label and a `st.popover` ("View system prompt") showing the exact, full system prompt actually sent to Claude for that tab/mode combo, verbatim via `st.code(...)`. Chose `st.popover` over this page's usual `fn-wrap`/`fn-tooltip` hover-tooltip canon specifically because the canon's `.fn-tooltip` has a hardcoded `width:280px` — far too small for a multi-paragraph system prompt — and the user explicitly wanted native scroll rather than a one-off wide-tooltip CSS variant. State is per-tab (`mode_key = f"mode_{corpus['key']}"` in `st.session_state`), matching the per-tab conversational memory pattern already in place — each corpus tab can be in a different mode simultaneously. Not added to the Agentic RAG tab (out of scope for this pass, `AGENTIC_SYSTEM_PROMPT` untouched). Verified live via Playwright: toggle switches state and the "Active" label correctly, popover opens and shows the complete, non-truncated prompt text (confirmed exact string match against the real `SYSTEM_PROMPT` constant).

- **DECIDED AGAINST (2026-08-01) — `ask_claude()` (`_0010_data.py:121-137`) doesn't set `temperature`, silently using the API default of 1.0.** Confirmed by reading the code: no `temperature` key in the payload. Originally flagged as misaligned with the system prompt's grounding instruction. Revisited and deliberately left as-is per the user: the system prompt is already tightly constrained ("ONLY the provided context... say so if it doesn't contain the answer"), so keeping the higher default temperature gives useful behavioral variability to observe and debug against, rather than optimizing blind toward determinism before seeing how it actually behaves. Not a bug — a conscious choice, revisit only if real output quality issues show up in testing.
...
- **2026-08-02 — RAG #2 (The Pienza Papers) has a retrieval-recall gap: right abstraction-level phrasing needed for top-k to surface the relevant chunk.** Full repro and diagnosis in `incidents_log.md` (2026-08-02 entry) — not a bug, the grounding/honesty behavior was correct both times, but "How did the project manage the bias-variance tradeoff?" missed the actual relevant passage (the lightweight 6-feature model / generalization-gap discussion) until rephrased as "nothing on a lightweight model?" in the same thread. Root cause: plain top-k=4 cosine similarity has no query expansion or keyword fallback, so a query using different vocabulary than the source passage can miss it even when the passage directly answers the question. **Not fixed now, deliberately** — candidates already on record: hybrid search (BM25 + vector, cross-corpus item, see the standing "hybrid search en todo" note) and/or raising `TOP_K` specifically for smaller/denser corpora like The Pienza Papers. Revisit alongside the hybrid-search work, not as a standalone fix.

- **DONE (2026-08-02) — RAG evaluation Phase 0: golden dataset + baseline retrieval-hit-rate + faithfulness harness.** Resolves the earlier "no scope defined yet" placeholder. Built as the first step of an explicitly incremental plan: query rewriting -> hybrid search (BM25+vector) -> reranking, each to be measured against this SAME golden dataset before/after, instead of adding all three blind ("no meter todo a una licuadora" — user's framing). Full design in the approved plan (now implemented).
  1. **Golden dataset** (`observatory/scripts/eval_data/rag_golden_dataset.py`): reused the 24 existing `SAMPLE_QUESTIONS` (6/corpus), each manually verified against real corpus content (grep against `.claude/claude_docs/*.md`, `git show origin/paper-dev:...` for the 10 `.tex` chapters, live `retrieve()`/`retrieve_trips()` calls against the real Codebase parquet and Trip Records ChromaDB) to confirm true `expected_source`/`expected_chunk_id` — not "whatever retrieval currently returns" (that would make hit-rate tautologically 100%). 2 Trip Records questions were swapped for cleaner single-row ground truth (the originals were structurally unanswerable: "fare range for accepted offers" is an aggregation question no top-k sample can answer — the same gap already documented as the "RAG Limitation" tooltip on the live page — and "compare accepted vs rejected" had a real top-4 that returned 4 rejected rows, zero accepted). Two genuine baseline misses were found and kept, not hidden: a Codebase question where the correct chunk ranks 3rd with a thin margin, and one where plain `retrieve()` returns the wrong specific chunk (`module-level` instead of `function build_sidebar`) though it still registers as a source_file-level hit — flagged in the dataset's own docstring as a known limitation of source_file-level (vs. chunk-level) hit-rate.
  2. **`rag_eval.py`** — retrieval-only baseline script, 100% reuses the live `retrieve()`/`retrieve_trips()` functions from `_0010_data.py` (never reimplements retrieval, so the eval can't silently drift from production behavior). `--config` flag (default `baseline`) is the extension point for later phases. Never calls `ask_claude()` — kept near-free to re-run. **Real baseline result: 91.7% hit-rate, $0.000032 total embed cost, confirms the two already-known misses** (the Vertex AI 20K-token-ceiling question in Project Docs, the Cognitive-Cascade/Gravitational-Well question in The Pienza Papers — both previously found live, now reproduced by the harness). Output: `data/dumped_files/rag_eval_runs/2026-08-02_baseline.json`.
  3. **RAGAS attempted, then abandoned after 3 real failed installs — genuine upstream bug, not a project-side choice.** Plan's explicit decision was to override this project's no-SDK precedent and use RAGAS + the `anthropic` SDK for faithfulness/answer-relevancy/context-precision/context-recall. Installed cleanly (`ragas==0.4.3`, pulls in `langchain`, `langchain-core`, `langgraph`, `openai`, `instructor`, `datasets` — a genuinely heavy tree, as anticipated), but `ragas.llms.llm_factory` unconditionally imports `langchain_community.chat_models.vertexai` at module load time, in **every** ragas version tested (0.4.3, 0.3.9, 0.2.15) — and that submodule does not exist in the only installable `langchain-community` release (0.4.2; the package itself is officially sunset/deprecated). Installing the real Vertex AI SDK (`langchain-google-vertexai`) does not fix it — the missing module belongs to a different, unrelated package. No older `langchain-community` version is available on PyPI to downgrade into. Confirmed via direct inspection of `ragas/llms/base.py`'s import block (not guessed) — this is RAGAS's own dependency being broken against its own dependency's current state, unrelated to whether Claude vs. OpenAI vs. anything else is the target LLM. Fully uninstalled (`ragas`, the entire `langchain*` tree, `anthropic`, `openai`, `instructor`, `datasets` — ~20 packages) — zero trace left in either `requirements.txt` (root or `observatory/`), confirmed via `pip list`/`grep`.
  4. **`rag_eval_judge.py`** — Path A fallback (hand-written Haiku judge), built after the RAGAS failure, not before — reuses this project's own `_claude_request()`/`ask_claude()` verbatim, a simple 1-5 faithfulness-scoring judge prompt, zero new dependencies. **Real result: 4.75/5 mean faithfulness across all 24 questions** (claude_docs 4.5, paper 4.67, trips 4.83, codebase 5.0). **Found one real, actionable issue**: the anonymization question scored 2/5 because the *retrieved context itself* was stale — `prompt_engineering.md` still said fares should use `█` block-masking, while root `CLAUDE.md` had already documented the switch to `#` per-digit masking as canonical (a change made in an earlier session, never backported to `prompt_engineering.md`). Claude's generated answer was actually correct (used `#`, the real current convention) but the judge correctly flagged it as unfaithful *to the stale retrieved passage* — exactly the kind of "faithful to wrong context" bug a faithfulness judge is supposed to catch. **Fixed same day**: `prompt_engineering.md` updated to say `#` per-digit, with a cross-reference to root `CLAUDE.md`; will self-correct in the corpus on the next nightly GitHub Action rebuild. Output: `data/dumped_files/rag_eval_runs/2026-08-02_baseline_judge.json`.
  5. **Real cost, confirmed vs. estimated**: embed cost $0.000032 (estimate was ~$0.037 — the actual 24 real questions were shorter than the estimate's assumed average). Judge run (24x `ask_claude()` + 24x judge call, 48 Haiku calls total) completed in ~2 minutes of wall-clock time; exact $ cost not separately logged this run (small enough, well under the ~$0.30 upper-bound estimate, per Haiku 4.5 pricing) — worth adding an actual `cost_usd` field to `rag_eval_judge.py`'s output in a future pass if tighter cost tracking is wanted.
  6. **Deferred, not built now**: Phases 1-3 (query rewriting, hybrid search BM25+vector, reranking) — each will plug into `rag_eval.py --config <name>` per the extension point already built, re-running this same golden dataset and writing its own dated JSON so hit-rate/faithfulness/cost/latency are comparable apples-to-apples across configs before any is adopted into the live page.
- **DONE (2026-08-03) — RAG evaluation Phases 1-3 (query rewriting, hybrid search, reranking) + full cost/metric breakdown + live "RAG Eval" tab on 0010.** Completed the incremental plan Phase 0 deferred. All 3 configs measured against the same 24-question golden dataset via `rag_eval.py --config <name>` + `rag_eval_judge.py --config <name>`, aggregated by a new `rag_eval_compare.py`. Full write-up with per-phase adoption reasoning: `.claude/claude_docs/rag_eval_workflow.md`. Methodology documented as a reusable concept in `agentic_knowledge.md` (2026-08-03 entry).
  1. **Context precision/recall added to `rag_eval.py`** (single-ground-truth-label simplification, documented explicitly: recall = hit 0/1, precision = 1/rank if hit). Re-run baseline with all 3 retrieval metrics together: 95.8% hit-rate, 0.931 precision, 0.958 recall.
  2. **`rag_eval_judge.py` rewritten to capture real Anthropic `usage` token counts** (not estimated) and split generation cost vs. judge cost. Real baseline: 4.67/5 faithfulness, $0.115488 generation + $0.065584 judge = $0.181072 total.
  3. **Phase 1 — query rewriting** (`rewrite_query()` in `_0010_data.py`, one Haiku call resolving pronouns/references against prior turns). Standalone questions unaffected (95.8%, identical to baseline, expected — no history to rewrite). The real test is 4 new golden follow-up pairs (`GOLDEN_FOLLOWUPS` in `rag_golden_dataset.py`, one per corpus, each manually verified live before/after): **follow-up hit-rate 25% -> 75%** with rewriting. **Strongest result of the 3 phases — adopted as the recommendation.**
  4. **Phase 2 — hybrid search** (`retrieve_hybrid()`, BM25 + cosine blend via `rank_bm25`, `alpha=0.5` untuned). **Real, honest negative result, not smoothed over**: 91.7% hit-rate (worse than baseline's 95.8%), 0.826 precision (worse), +36% latency, higher Anthropic cost (longer generated answers). New miss appeared that baseline didn't have ("How is address anonymization implemented?", codebase). **Not adopted** — documented as a genuine finding, with untuned alpha and naive `.lower().split()` BM25 tokenization flagged as likely (uninvestigated) causes for a future pass.
  5. **Phase 3 — reranking** (`retrieve_with_rerank()`, real local cross-encoder `cross-encoder/ms-marco-MiniLM-L-6-v2` via `sentence-transformers`, PyTorch inference — architecturally distinct from an LLM-based rerank, no API call). Wide-retrieve-then-rerank-narrow pattern (`wide_k=15` -> `k=4`). Ties baseline hit-rate (95.8%) with a different single miss, +51% latency (7996ms vs 5301ms), zero marginal API cost. **Neutral, not adopted** — no measurable net gain on this 24-question dataset to justify the new PyTorch dependency and latency cost.
  6. **Real bug found and fixed during the judge runs**: `rag_eval_judge.py`'s `retrieve_for_config()` for `--config hybrid` called `retrieve_hybrid()` without the required `corpus_file` argument, causing `_load_bm25_index()` to return `None` and an `AttributeError` on first run. `rag_eval.py`'s dispatcher already had the correct pattern (`corpus_file` looked up from `CORPORA` by key); replicated the same fix in `rag_eval_judge.py`. Confirmed fixed and working on re-run.
  7. **`rag_eval_compare.py`** (new): aggregates the latest run per config from `data/dumped_files/rag_eval_runs/*.json` (retrieval + judge) into one comparison table, plus the corpus-build-cost table for all 4 corpora (computed for the first time — Project Docs $0.005991, The Pienza Papers $0.003942, Trip Records $0.024929, Codebase $0.018356, total $0.053218) and the follow-up raw-vs-rewritten comparison. Writes `_comparison.json`.
  8. **Live "RAG Eval" tab added to 0010** (`st.tabs(["Manual Retrieval", "Agentic RAG", "RAG Eval"])`, new `_render_eval_tab()`). Read-only — reads `rag_eval_comparison.json` from GCS via a new `load_eval_comparison()` cached loader in `_0010_data.py`, never runs an eval live. New `gcs_deploy.py` MANIFEST entry, uploaded with explicit user permission per the repo's GCS-only rule. **Verified live via Playwright**: all 3 outer tabs render correctly (including the new one), zero page errors, real numbers match the JSON exactly, Manual Retrieval's stepper and Agentic RAG tab confirmed unaffected by the third tab's addition (the stepper's tab-index-dependent JS was already correctly scoped to its own tab panel, so no fix was needed there).
- **2026-08-01 — LangChain exploration, no scope defined yet.** Same origin as the RAG evaluation item above (weekend/future list, never formally logged). Intent was to explore LangChain as a comparison point against this project's own from-scratch RAG implementation (manual cosine similarity, manual chunking, manual tool-use loop) — not yet decided whether that means rebuilding a piece of 0010 with LangChain, or just a standalone study exercise outside the app. Placeholder only.
...
- **ABANDONED (2026-08-02) — ChromaDB corpus (candidate #4, Trip Records tab in 0010) felt slow, flagged for profiling 2026-07-30.** User confirmed this is no longer happening — not reproducible anymore, no fix was ever applied, symptom just isn't present in current usage. Not root-caused (never confirmed whether it was the multi-file GCS download, ChromaDB warm-up, or just cold-session latency stacking), but no longer worth tracking since there's nothing to profile.
- **RESOLVED (2026-08-01) — RAG system prompt (`ask_claude()` in `_0010_data.py`) needed a prompt-engineering pass.** Done as part of the conversational-memory work (see the DONE item above for the full rewrite description). Decision: kept as one shared, parametrized-by-context prompt (`SYSTEM_PROMPT`) rather than splitting per corpus — the prompt itself disambiguates the 3 corpora and instructs the model never to mix them, so a single well-structured prompt covers all 3 tabs without duplication. The "documentation assistant" mislabeling for the Trip Records tab is fixed — the new prompt describes all 3 corpora accurately, no longer defaults to a docs-only framing.
- **2026-07-30 — Create hooks and agents for this project.** Raised during a Claude Code documentation study session (features-overview, tools-reference). This repo has no custom `.claude/agents/*.md` subagent definitions and no project hooks in `settings.json` yet — both are documented capabilities the user hasn't set up locally. Not scoped/prioritized yet; revisit once there's a concrete recurring task that fits either pattern (per the "build your setup over time" trigger table — don't add either speculatively).
- **2026-07-30 — Refactor/reorganize `CLAUDE.md` and `assets_ignored/claude_docs/`.** Also surfaced during a Claude Code documentation study session — the docs recommend keeping CLAUDE.md under ~200 lines and moving reference material out into skills/rules — this project's CLAUDE.md is currently 461 lines covering many unrelated topics (GCS deployment workflow, anonymization protocol, BQ reference, environment mechanics, STAR stories pointer, etc.) that could arguably live in `.claude/rules/` (path-scoped) or dedicated skills instead. Not scoped yet — needs a deliberate pass, not a drive-by edit.
